...
# @component
#
# Creates and formats a new Next.js component in the specified directory.
# Usage: @component [add|create] NAME [OPTIONS]
...
error() {
  [ -z "$ERROR_ICON" ] && ERROR_ICON="✖"
  printf "%s %s\n\n" "$(red "$ERROR_ICON")" "$1"
  [ "$2" = "--exit" ] && exit 1
}
success() {
  [ -z "$SUCCESS_ICON" ] && SUCCESS_ICON="✔"
  printf "%s %s\n\n" "$(green "$SUCCESS_ICON")" "$1"
  [ "$2" = "--exit" ] && exit 0
}
is_kebab_case() {
  echo "$1" | grep -q "^[a-z-]*$"
...
camel_case() {
  input=$(pascal_case "$1")
  echo "$(printf "%s" "$input" | cut -c 1 | tr "[:upper:]" "[:lower:]")$(printf "%s" "$input" | cut -c2-)"
}
